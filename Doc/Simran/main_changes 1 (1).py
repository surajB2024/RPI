...
class SerialUtility:
    def __init__(self, root):
        ...
        self.log_console = tk.Text()  # Initialize your Tkinter Text widget here
        self.log_file = None
    ...
    def insert_ansi_colored_text(self, text):
        # Insert text into the log console and set the flag if user scrolls up
        self.log_console.insert(tk.END, text + '\n')
        self.log_console.bind("<MouseWheel>", self.on_mouse_wheel)

    def on_mouse_wheel(self, event):
        # Allow free scrolling; set flag when scrolling up
        if event.delta > 0:  # Scrolling up
            self.user_scrolled_up = True
        elif event.delta < 0:  # Scrolling down
            # Optionally reset the flag when scrolling down
            self.user_scrolled_up = False
    ...
    def insert_ansi_colored_text(self, text):
        ...
        for part in parts:
            ...
            if ansi_escape.match(part):
                ...
                # Insert the text part with the current color tagau
                if current_tag:
